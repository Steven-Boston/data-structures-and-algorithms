...
const minutes = (timeString) => {
  return 60*Number(timeString.substring(0,2))+Number(timeString.substring(2,4))
}

const sortSchedule = (arr) => {
  let dayIndex = ['Monday','Tuesday','Wednesday','Thursday','Friday'];
  let arrDayIndex = arr.map(meeting => dayIndex.indexOf(meeting.dayOfWeek));
  let arrLengthIndex = arr.map(meeting => minutes(meeting.end)-minutes(meeting.start));
  return arr.sort((a,b) => {
    if (arrDayIndex[arr.indexOf(a)]-arrDayIndex[arr.indexOf(b)]>0) {
      return 1;
    } else if (arrDayIndex[arr.indexOf(a)]-arrDayIndex[arr.indexOf(b)]<0) {
      return -1;
    } else if (Number(a.start)-Number(b.start)>0) {
      return 1;
    } else if (Number(a.start)-Number(b.start)<0) {
      return -1;
    } else if (arrLengthIndex[arr.indexOf(a)]-arrLengthIndex[arr.indexOf(b)]>0) {
      return 1;
    } else if (arrLengthIndex[arr.indexOf(a)]-arrLengthIndex[arr.indexOf(b)]<0) {
      return -1;
    } else {
