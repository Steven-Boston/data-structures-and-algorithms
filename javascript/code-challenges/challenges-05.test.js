...
const returnNames = (arr) => {
  let names = [];
  arr.reduce((acc,value)=>names.push(value.name),[]);
  return names;
};
...
const reversedString = (str) => {
  return [...str].reduce((word,letter) => `${letter}${word}`,'');
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((kids,char)=> kids + (char.children ? char.children.length : 0),0);
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((primes,num)=> isPrime(num) ? primes+1 : primes,0);
};
...
const extractStat = (statName, arr) => {
  return arr.reduce((acc,stat)=> stat.stat.name === statName ? acc=stat : acc=acc , null );
};
...
const extractChildren = (arr) => {
  ...
  return aChars.reduce((acc,char)=>char.children ? acc = [...acc,...char.children] : acc=acc,[]);
};
