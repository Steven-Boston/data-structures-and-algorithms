...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(string=> {
     let tempString = string.slice(string.indexOf(' ')+1);
     result.push(tempString.slice(tempString.indexOf(' ')+1));
  });
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(string=>{result.push(joinArray(string.split(' ').splice(2)))});
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  recipe.steps.forEach(step=> result.push(step.split(' ').splice(0,1)[0]));
  return result;
};
...
const removeEvenValues = (arr) => {
  arr.splice(0,arr.length,...arr.filter(num=>num%2==1));
};
...
const totalSumCSV = (str) => {
  let total = 0;
  str.split(',').forEach(num=>total+= Number(num));
  return total;
};
...
const removeVowels = (str) => {
  ...
  let stringList = [...str];
  vowels.forEach(vowel=>{stringList = stringList.filter(letter=>letter !== vowel)});
...
const extractVowels = (str) => {
  ...
  let stringList = [...str];
  vowels.forEach(vowel=>{stringList = stringList.filter(letter=>letter !== vowel)});
  let vowelList = [...str].filter(letter=>!stringList.includes(letter)).sort();
  return [stringList.join(''),vowelList.join('')];
};
