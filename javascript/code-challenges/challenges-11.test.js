...
const count = (target, input) => {
  return input.reduce((arr, piece)=>arr=[...arr,...piece],[]).reduce((count,num)=>(num==target)?count=count+1:count,0);
};
...
let findShortest = (data) => {
  return data.reduce((short, next)=>short.height<next.height?short=next:short,{height:0}).name
};
