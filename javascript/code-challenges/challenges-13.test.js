...
const longestString = (arr) => {
  ...
  return arr.reduce((ind,string)=>string.length>arr[ind].length ? ind=arr.indexOf(string) : ind,0);
  };
};
...
const standardizePhoneNumbers = (arr) => arr.map(str=>[...str].map(ch=>parseInt(ch)).filter(ch=>ch+1).join(''));
...
const allHappy = (arr) => arr.reduce((bool,str)=>/\:\)/.test(str) ? bool : bool=false,true);
...
const findAnything = (arr, target) => arr.filter(word=>word.includes(target));
...
const findEvery = (arr, target) => findAnything(arr,target).length==arr.length;
...
const unenrollBrook = (arr) => arr.map(arr=>arr.filter(str=>!str.includes('Brook')));
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach((day,ind)=> {
    schedule.push([]);
    arr.forEach(appt=> {
      appt.includes(day) ? schedule[ind].push(appt) : '';
    });
  });
  return schedule;
};
