...
const hasChildrenEntries = (arr, character) => {
  return Object.entries(arr.find(char=>char.name===character)).find(entry=>entry.includes('children')) ? true : false;
};
...
const totalCharacters = (arr) => {
  return characters.map(char=>1+(char.spouse?1:0)+(char.children?char.children.length:0)).reduce((acc,num)=>acc+num,0);
};
...
const houseSize = (arr) => {
  return arr.map(char=>{
    return {
    'house':char.house,
    'members':1+(char.spouse?1:0)+(char.children?char.children.length:0)
    }
  });
};
...
const houseSurvivors = (arr) => {
  return arr.map(char=>{
    return {
    'house':char.house,
    'members':1+(char.spouse?deceasedSpouses.includes(char.spouse)?0:1:0)+(char.children?char.children.length:0)
    }
  });
};
