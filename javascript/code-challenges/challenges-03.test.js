...
const typeNum = (arr) => {
  return arr.filter(i => i === Number(i));
};
...
const getStatName = (arr, minBaseStat) => {
  return getBaseStatGreaterThan(arr, minBaseStat).map(statObject => statObject.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  return typeNum(arr).map(num => num % 2 === 1 ? 'odd' : 'even');
};
