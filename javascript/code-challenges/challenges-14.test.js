...
const screenForNames = (arr) => arr.filter(str=>/^(Mr|Mrs|Ms|Dr). [a-zA-Z]+$/.test(str));
...
//Just checking how close attention y'all are paying. A real answer:
let reallyBiggerThanLuke = (arr) => arr.filter(char=>Number(char.mass)>Number(arr[0].mass)).map(char=>char.name).join(' - ');
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => /^https\:\/\/s/.test(url);
...
const helpCheck = (board, row1, col1, row2, col2, row3, col3) => {
  let points = [board[row1][col1],board[row2][col2],board[row3][col3]]
  return ((points[0]==points[1]&&points[1]==[points[2]])&&(points[0])) ? true : false; 
}
...
const detectTicTacToeWin = (board) => {
  ...
  return lines.reduce((win,line)=>helpCheck(board,...line)?win=true:win,false);
};
